...
def buat_tabel_rbd_crd(data):
    ...
    for block in range(1, 4):
        ...
        run_order_start = (block - 1) * len(combinations) + 1
        
        for i, idx in enumerate(indices):
            komposisi, kompaksi, cavity, frekuensi = combinations[idx]
            specimen_values = data[idx][4:9]
            overall_avg = data[idx][9]
            
            run_order = run_order_start + i
            rbd_data.append([std_order, run_order, block, komposisi, kompaksi, cavity, frekuensi] + specimen_values + [overall_avg])
            std_order += 1
            
            if block == 1:
                crd_data.append([i+1, i+1, 1, komposisi, kompaksi, cavity, frekuensi] + specimen_values + [overall_avg])
    
    rbd_df = pd.DataFrame(rbd_data, columns=[
        'StdOrder', 'RunOrder', 'Blocks', 'Komposisi', 'Kompaksi', 'Cavity', 'Frekuensi',
        '1.00', '2.00', '3.00', '4.00', '5.00', 'Rata-rata Koefisien'
    ])
...
def main():
    ...
    print(f"Total CRD: {len(crd_df)} baris (108 data frekuensi + 27 data rata-rata)")
